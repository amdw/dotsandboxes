...
In previous sections, we have built up a considerable body of
knowledge about strings-and-coins. In order to improve our
understanding further, it is now necessary to look at the game in the
context of the broader mathematical theory of games of this type.

There is a rich mathematical literature on Combinatorial Game Theory,
or CGT. The key founding text of this theory is \cite{onag}, but the
best introductory survey I am aware of is Richard Guy's papers in
\cite{nochance}. \cite{lip} is also an excellent introductory
textbook.
...
the player who takes the \emph{last} coin. Nimstring satisfies all the
conditions above and is therefore ideally suited to analysis using
CGT.
...
G^{R_2}, \ldots\}\}$$ where $G^{L_1}$ etc are the games (with $R$ to
move) which $L$ can choose to move to if it is $L$'s move, and
$G^{R_1}$ etc are the games (with $L$ to move) which $R$ can choose to
move to if it is $R$'s move. These two sets of options can be empty,
finite or infinite.
...
called the \emph{zero game} and is often just written as $0$. So the
game $\{0\:|\:\}$ is the game where $L$ has one legal move leading to
the zero game and $R$ has no legal moves; $\{\:|\:0\}$ is the game
where $R$ has one legal move leading to the zero game and $R$ has no
legal moves, etc.
