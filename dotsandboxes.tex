...
\begin{enumerate}
  ...
  \item The players take it in turns to remove a string of their
    choice.
...
\end{dnbgamelength}
\begin{proof}
  If the dots-and-boxes game is played on a grid of $m \times n$
  boxes, the number of coins in the equivalent strings-and-coins
  position is $mn$, and the number of strings is $$m(n+1) + n(m+1)$$

  Substituting these numbers into Theorem \ref{gamelength} and
  simplifying shows that the game will have $$(m+1)(n+1) + D$$ turns,
  and since the board has $(m+1)(n+1)$ dots, we have our result.
\end{proof}
