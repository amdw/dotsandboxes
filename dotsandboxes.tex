...
dots-and-boxes, there are no loops with fewer than four coins, and due to
...
position to the opponent, and so in a practical game, the first player
to force their opponent to make a loony move is almost always the
winner. (We will discuss the exceptions shortly.)
...
\begin{itemize}
  ...
  \item All loony Nimstring positions are a win for the first player
    (similar to Theorem \ref{sstealing}). However in Nimstring, since there
    is no score-keeping, there are no special situations where you win by
    playing a loony move which wins more coins than it sacrifices.
